...
class SqlConnector(
    private val url: String = "jdbc:postgresql://localhost:5432/autoshopdevdb",
    private val user: String = "postgres",
    private val password: String = "autoshop-pass",
    private val schema: String = "autoshop",
    ...
    fun connect(vararg tables: Table): Database {
        ...
        transaction(connect) {
            if (System.getenv("as.sql_drop_db")?.toBoolean() == true) {
                SchemaUtils.drop(*tables, inBatch = true)
                SchemaUtils.create(*tables, inBatch = true)
            } else if (System.getenv("as.sql_fast_migration").toBoolean()) {
