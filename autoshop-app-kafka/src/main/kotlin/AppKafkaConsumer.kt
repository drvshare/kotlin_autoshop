...
import ru.drvshare.autoshop.common.helpers.KLOGGER
...
interface ConsumerStrategy {
    ...
    fun serialize(source: AsAdContext): String
    fun deserialize(value: String, target: AsAdContext)
}

class AppKafkaConsumer(
    private val config: AppKafkaConfig,
    consumerStrategies: List<ConsumerStrategy>,
    private val processor: AsAdProcessor = AsAdProcessor(),
    ...
    fun run() = runBlocking {
        try {
            ...
            while (process.value) {
                ...
                val records: ConsumerRecords<String, String> = withContext(dispatcher) {
                    consumer.poll(Duration.ofSeconds(1))
                }
                if (!records.isEmpty)
                    KLOGGER.info { "Receive ${records.count()} messages" }

                records.forEach { record: ConsumerRecord<String, String> ->
                    try {
                        KLOGGER.info { "process ${record.key()} from ${record.topic()}:\n${record.value()}" }
    ...
    private fun sendResponse(context: AsAdContext, strategy: ConsumerStrategy, outputTopic: String) {
        ...
        KLOGGER.info { "sending ${resRecord.key()} to $outputTopic:\n$json" }
